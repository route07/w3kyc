...
// Contract service class for all 19 contracts
export class Web3ContractServices {
  ...
  constructor() {
    ...
    // Get private key from environment with fallback
    const privateKey = process.env.PRIVATE_KEY || '0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80';
    
    // Validate private key format
    if (!privateKey || !privateKey.startsWith('0x') || privateKey.length !== 66) {
      throw new Error(`Invalid private key format: ${privateKey ? privateKey.substring(0, 10) + '...' : 'undefined'}`);
    }
    
    try {
      this.signer = new ethers.Wallet(privateKey, this.provider);
      console.log('✅ Web3ContractServices initialized with wallet:', this.signer.address);
    } catch (error) {
...
export const web3ContractServices = new Web3ContractServices();