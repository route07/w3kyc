...
# This script helps maintain and update documentation without external dependencies

set -e

echo "🔧 Web3 KYC Documentation Update Script (Simple)"
echo "================================================"
...
check_internal_links() {
    ...
    # Check for markdown files that don't exist
    for file in docs/**/*.md; do
        if [ -f "$file" ]; then
            # Extract links to other markdown files
            while IFS= read -r line; do
                if [[ $line =~ \[([^\]]+)\]\(([^)]+\.md)\) ]]; then
                    link_file="${BASH_REMATCH[2]}"
                    # Handle relative paths
                    if [[ $link_file == docs/* ]]; then
                        if [ ! -f "$link_file" ]; then
                            print_warning "Broken link in $file: $link_file"
                            BROKEN_LINKS=$((BROKEN_LINKS + 1))
                        fi
                    elif [[ $link_file == ../* ]]; then
                        # Handle relative paths from docs subdirectories
                        link_file="docs/$link_file"
                        if [ ! -f "$link_file" ]; then
                            print_warning "Broken link in $file: $link_file"
                            BROKEN_LINKS=$((BROKEN_LINKS + 1))
                        fi
