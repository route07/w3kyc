...
async function main() {
  console.log("🚀 Starting Tractsafe core deployment...");
  ...
  try {
    ...
    // Deploy core contracts that we know work
    ...
    // 5. Deploy ComplianceChecker with storage addresses
    ...
    const complianceChecker = await ComplianceChecker.deploy(
      ...
      "0x0000000000000000000000000000000000000000", // Placeholder for DIDCredentialStorage
      {
        gasLimit: 3000000,
        gasPrice: ethers.parseUnits("1", "gwei"),
      }
    ...
    // 6. Deploy DocumentManager (no constructor args)
    ...
    // 7. Deploy RiskAssessment (no constructor args)
    ...
    // 8. Deploy AuditLogger (no constructor args)
    ...
    // 10. Deploy KYCManager with all dependencies
    ...
    // Write to file
    const deploymentFile = path.join(__dirname, '..', 'deployments', 'tractsafe-core-deployment.json');
    ...
    console.log("\n🎉 Core contracts deployed successfully to Tractsafe network!");
