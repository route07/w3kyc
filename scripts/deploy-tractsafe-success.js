...
async function main() {
  console.log("🚀 Starting Tractsafe successful deployment...");
  ...
  try {
    ...
    // Deploy contracts that we know work
    ...
    // 6. Deploy ComplianceChecker with storage addresses (using zero address for DIDCredentialStorage)
    ...
    const complianceChecker = await ComplianceChecker.deploy(
      ...
      "0x0000000000000000000000000000000000000000", // Zero address for DIDCredentialStorage
      {
        gasLimit: 3000000,
        gasPrice: ethers.parseUnits("1", "gwei"),
      }
    ...
    // Write to file
    const deploymentFile = path.join(__dirname, '..', 'deployments', 'tractsafe-success-deployment.json');
